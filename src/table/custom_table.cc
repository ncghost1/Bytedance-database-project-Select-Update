...
int64_t CustomTable::PredicatedColumnSum(int32_t threshold1, int32_t threshold2) {
    // TODO: Implement this!
    int64_t sum = 0;
    ...
    if (num_cols_ >= 3) {
        ...
        for (auto row_id : row_id_group) {
            ...
            if (val_c2 < threshold2) {
                auto offset_c0 = col0 * FIXED_FIELD_LEN * num_rows_ + row_id * FIXED_FIELD_LEN;
                auto val_c0 = *(int32_t*)(columns_ + offset_c0);
                sum += val_c0;
            }
        }
    }
